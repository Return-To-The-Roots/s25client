...
- `AIPlayerJH::ComputeFulfillmentLevel` (`libs/s25main/ai/aijh/runtime/AIPlayerJH.cpp:1708-1764`) scores how ready each frontline building is to launch an attack. The AI iterates all military buildings returned by `aii.GetMilitaryBuildings()`, filters down to established ones (`!IsNewBuilt()`) that sit on a near frontier, and skips entries where the flag or attacker list is missing.
