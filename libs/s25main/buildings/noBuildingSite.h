...
public:
    ...
    noBuildingSite(const BuildingType type, const MapPoint pos, unsigned char player);
    /// Konstruktor für Hafenbaustellen vom Schiff aus
    noBuildingSite(const MapPoint pos, unsigned char player);
...
public:
    ...
    unsigned CalcDistributionPoints(noRoadNode* start, const GoodType goodtype);
