...
void Scaler::Scale(std::vector<unsigned char>& heightMap)
{
    ...
    {
        auto norm = double(heightMap[i] - minimum) / (maximum - minimum);
        auto height = char(height_.minimum + round(norm * diff));
        
        heightMap[i] = height;
    }
}

std::vector<unsigned char> Scaler::Scale(const std::vector<int>& map)
{
    ...
    for (int i = 0; i < size; ++i)
    {
        auto norm = maxDiff != 0 ? double(map[i] - minimum) / maxDiff : 0.0;
        
        heightMap[i] = int(height_.minimum + round(norm * diff));
    }
