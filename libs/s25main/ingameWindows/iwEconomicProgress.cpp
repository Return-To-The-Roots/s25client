...
iwEconomicProgress::iwEconomicProgress(const GameWorldViewer& gwv)
    : IngameWindow(CGI_ECONOMICPROGRESS, IngameWindow::posLastOrCenter,
                   Extent(240, 96 + 26 * EconomyModeHandler::numGoodTypesToCollect),
                   _("Economic Progress"),
      ...
      gwv(gwv)
{
    ...
    headline = AddText(1, DrawPoint(11 + 27, 46), _("Player"), COLOR_GREEN,
    ...
    {
        if(teams[i].mask & getPlayerMask(gwv.GetPlayer().GetPlayerId()))
        {
            mainTeam = i;
            break;
        }
    ...
    AddText(2, DrawPoint(11 + 30, 240), _("Percent elapsed:"), COLOR_ORANGE,
                       FontStyle::LEFT | FontStyle::TOP, NormalFont);
    elapsedTime = AddText(3, DrawPoint(30 + 63 * 3, 240), "%", COLOR_ORANGE,
            FontStyle::RIGHT | FontStyle::TOP, NormalFont);
...
void iwEconomicProgress::Draw_() {
    ...
    for (unsigned int t = 0; t < teams.size(); t++) {
        ...
        {
            if(teams[teamorder[t]].mask & getPlayerMask(i))
            {
