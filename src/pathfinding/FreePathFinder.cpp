                                                   ...
                                                   FP_Node_OK_Callback IsNodeOK, FP_Node_OK_Callback IsNodeOKAlternate, FP_Node_OK_Callback IsNodeToDestOk, const void* param)
{
    if(start == dest)
    {
        ...
        RTTR_Assert(false);
        LOG.writeCFormat("WARNING: Bug detected (GF: %u). Please report this with the savegame and replay (Start==Dest in pathfinding %u,%u)\n", gwb_.GetEvMgr().GetCurrentGF(), unsigned(start.x), unsigned(start.y));
    ...
    //LOG.writeCFormat(("pf: from %i, %i to %i, %i \n", x_start, y_start, x_dest, y_dest);
    ...
    {		
        ...
        {			
            ...
            //prevstepEven ? LOG.writeCFormat(("pf: even, to switch %i listsize %i ", stepsTilSwitch, todo.size()) : LOG.writeCFormat(("pf: odd, to switch %i listsize %i ", stepsTilSwitch, todo.size());
        }
        //else
        //prevstepEven ? LOG.writeCFormat(("pf: even, to switch %i listsize %i ", stepsTilSwitch, todo.size()) : LOG.writeCFormat(("pf: odd, to switch %i listsize %i ", stepsTilSwitch, todo.size());
        stepsTilSwitch--;
        ...
        //LOG.writeCFormat(("pf get neighbor nodes %i, %i id: %i \n", best.x, best.y, best_id);
        ...
        for(unsigned z = startDir + 3; z < startDir + 9; ++z)
        {
            ...
            if(nbId != destId && ((prevStepEven && IsNodeOK) || (!prevStepEven && IsNodeOKAlternate)))
            {
                ...
                else
                {
                    ...
                    bool tooClose = false;
                    //LOG.writeCFormat(("pf from %i, %i to %i, %i now %i, %i ", x_start, y_start, x_dest, y_dest, xa, ya);//\n
                    for(std::vector<MapPoint>::const_iterator it = evenLocationsOnRoute.begin();it!= evenLocationsOnRoute.end(); ++it)
                    {
                        //LOG.writeCFormat(("dist to %i, %i ", temp, *it);
