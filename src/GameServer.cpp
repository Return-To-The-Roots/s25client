...
bool GameServer::TryToStart(const CreateServerInfo& csi, const std::string& map_path, const MapType map_type)
{
    ...
    switch(mapinfo.type)
    {
        default: LOG.writeCFormat("GameServer::Start: ERROR: Map-Type %u not supported!\n", mapinfo.type); return false;
        ...
        case MAPTYPE_OLDMAP:
        {
            ...
            {
                LOG.writeCFormat("GameServer::Start: ERROR: Map \"%s\", couldn't load header!\n", mapinfo.filepath.c_str());
                return false;
            }
...
bool GameServer::Start()
{
    ...
    {
        LOG.writeCFormat("GameServer::Start: ERROR: Listening on port %d failed!\n", config.port);
    ...
    if(countdown.IsActive())
    {
        ...
        if(countdown.Update(VIDEODRIVER.GetTickCount()))
        {
            ...
            else
            {
                ...
                LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_SERVER_COUNTDOWN(%d)\n", countdown.GetRemainingSecs());
            }
        }
    }
...
void GameServer::Stop()
{
    ...
    // status
    status = SS_STOPPED;
    LOG.writeCFormat("server state changed to stop\n");
}
...
bool GameServer::StartCountdown()
{
    ...
    if(playerCount > 1)
    {
        ...
        SendToAll(GameMessage_Server_Countdown(countdown.GetRemainingSecs()));
        LOG.writeCFormatToFile("SERVER >>> Countdown started(%d)\n", countdown.GetRemainingSecs());
    }else if(!StartGame())
    {
        GAMEMANAGER.ShowMenu();
...
void GameServer::CancelCountdown()
{
    ...
    LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_SERVER_CANCELCOUNTDOWN\n");
}
...
bool GameServer::StartGame()
{
    ...
    LOG.writeCFormat("SERVER: Using gameframe length of %dms\n", framesinfo.gf_length);
    LOG.writeCFormat("SERVER: Using networkframe length of %u GFs (%ums)\n", framesinfo.nwf_length, framesinfo.nwf_length * framesinfo.gf_length);

    // Spielstart allen mitteilen
    SendToAll(start_msg);
    LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_SERVER_START(%d)\n", random_init);
    ...
    {
        LOG.writeCFormat("Error when loading game: %s\n", error.what());
        return false;
    }
    ...
    LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_NWF_DONE\n");
...
void GameServer::ChangeGlobalGameSettings(const GlobalGameSettings& ggs)
{
    ...
    LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_GGS_CHANGE\n");
}
...
void GameServer::KickPlayer(unsigned char playerId, unsigned char cause, unsigned short param)
{
    ...
    AnnounceStatusChange();
    LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_PLAYERKICKED(%d,%d,%d)\n", playerId, cause, param);
...
void GameServer::ClientWatchDog()
{
    ...
    if(set.Select(0, 2) > 0)
    {
        for(unsigned id = 0; id < players.size(); ++id)
        {
            ...
            {
                LOG.writeCFormat("SERVER: Error on socket of player %d, bye bye!\n", id);
...
void GameServer::ExecuteNWF(const unsigned  /*currentTime*/)
{
    ...
    for(unsigned playerId = 0; playerId < players.size(); ++playerId)
    {
        ...
        if(player.ps == PS_AI)
        {
            //LOG.writeCFormatToFile("SERVER >>> GC %u\n", playerId);
        ...
        {
            LOG.writeCFormat("Async at GF %u of players %u vs %u: Checksum %i:%i ObjCt %u:%u ObjIdCt %u:%u\n", currentGF, playerId, referencePlayerIdx,
...
void GameServer::CheckAndKickLaggingPlayer(const unsigned char playerIdx)
{
    ...
    if(timeOut == 0)
        ...
        LOG.writeCFormat("SERVER: Kicke Spieler %d in %u Sekunden\n", playerIdx, timeOut);
}
        ...
        if(set.InSet(serversocket))
        {
            ...
            for(unsigned playerId = 0; playerId < players.size(); ++playerId)
            {
                if(players[playerId].ps == PS_FREE)
                {
                    ...
                    //LOG.writeCFormat(("new socket, about to tell him about his playerId: %i \n",playerId);
                    // schleife beenden
                    break;
                }
            }
        ...
        if(set.Select(0, 0) > 0)
        {
            for(unsigned id = 0; id < players.size(); ++id)
            {
                if( players[id].isHuman() && set.InSet(players[id].so) )
                {
                    ...
                    {
                        LOG.writeCFormat("SERVER: Receiving Message for player %d failed, kick it like Beckham!\n", id);
                        KickPlayer(id, NP_CONNECTIONLOST, 0);
                    } else
                        msgReceived = true;
                }
...
inline void GameServer::OnGameMessage(const GameMessage_Player_Name& msg)
{
    ...
    LOG.writeCFormatToFile("CLIENT%d >>> SERVER: NMS_PLAYER_NAME(%s)\n", msg.player, msg.playername.c_str());
...
inline void GameServer::OnGameMessage(const GameMessage_Player_Set_Nation& msg)
{
    ...
    LOG.writeCFormatToFile("CLIENT%d >>> SERVER: NMS_PLAYER_TOGGLENATION\n", msg.player);

    // Nation-Change senden
    SendToAll(GameMessage_Player_Set_Nation(msg.player, msg.nation));

    LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_PLAYER_TOGGLENATION(%d, %d)\n", msg.player, player.nation);
}
...
inline void GameServer::OnGameMessage(const GameMessage_Player_Set_Team& msg)
{
    ...
    LOG.writeCFormatToFile("CLIENT%d >>> SERVER: NMS_PLAYER_TOGGLETEAM\n", msg.player);
    SendToAll(GameMessage_Player_Set_Team(msg.player, msg.team));
    LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_PLAYER_TOGGLETEAM(%d, %d)\n", msg.player, player.team);
}
...
inline void GameServer::OnGameMessage(const GameMessage_Player_Set_Color& msg)
{
    ...
    LOG.writeCFormatToFile("CLIENT%u >>> SERVER: NMS_PLAYER_TOGGLECOLOR %u\n", msg.player, msg.color);
...
inline void GameServer::OnGameMessage(const GameMessage_Player_Ready& msg)
{
    ...
    LOG.writeCFormatToFile("CLIENT%d >>> SERVER: NMS_PLAYER_READY(%s)\n", msg.player, (player.isReady ? "true" : "false"));
    // Broadcast to all players
    SendToAll(msg);
    LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_PLAYER_READY(%d, %s)\n", msg.player, (player.isReady ? "true" : "false"));
}
...
inline void GameServer::OnGameMessage(const GameMessage_Map_Checksum& msg)
{
    ...
    else
    {
        ...
        LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_PLAYER_NEW(%d, %s)\n", msg.player, player.name.c_str());
...
void GameServer::OnGameMessage(const GameMessage_SendAsyncLog& msg)
{
    if (msg.player == async_player1)
    {
        ...
        if (msg.last)
        {
            LOG.writeCFormat("Received async logs from %u (%lu entries).\n", async_player1, async_player1_log.size());
    ...
    {
        LOG.writeCFormat("Received async log from %u, but did not expect it!\n", msg.player);
        return;
    }
    ...
    LOG.writeCFormat("Async logs received completely.\n");
    ...
    LOG.writeCFormat("There are %u identical async log entries.\n", identical);
    ...
    {
        DebugInfo di;
        LOG.writeCFormat("Sending async logs %s.\n",
    ...
    if (file)
    {
        ...
        LOG.writeCFormat("Async log saved at \"%s\"\n", fileName.c_str());
    }
    else
    {
        LOG.writeCFormat("Failed to save async log at \"%s\"\n", fileName.c_str());
    }
...
void GameServer::CheckAndSetColor(unsigned playerIdx, unsigned newColor)
{
    ...
    LOG.writeCFormatToFile("SERVER >>> BROADCAST: NMS_PLAYER_TOGGLECOLOR(%d, %d)\n", playerIdx, player.color);
}
...
void GameServer::ChangePlayer(const unsigned char old_id, const unsigned char new_id)
{
    ...
    LOG.writeCFormat("GameServer::ChangePlayer %i - %i \n",old_id, new_id);
    using std::swap;
