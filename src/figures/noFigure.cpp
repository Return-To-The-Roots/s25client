...
// GrÃ¶ÃƒÆ’Ã‚Å¸e des Rechtecks um den Punkt, wo er die Flaggen sucht beim Rumirren
const unsigned short WANDER_RADIUS = 10;
/// Dasselbe nochmal fÃ¼r Soldaten
...

{
    ...
    // Haben wir ein Ziel?
    // Gehen wir in ein Lagerhaus? Dann dÃ¼rfen wir da nicht unsere Arbeit ausfÃ¼hren, sondern
...
void noFigure::StartWalking(const unsigned char dir)
{
    ...
    if(dir == 1 && gwg->GetNO(gwg->GetNeighbour(pos, 1))->GetType() == NOP_BUILDING)
        gwg->GetSpecObj<noBuilding>(gwg->GetNeighbour(pos, 1))->OpenDoor(); // Dann die TÃ¼r aufmachen
    // oder aus einem raus?
    if(dir == 4 && gwg->GetNO(pos)->GetType() == NOP_BUILDING)
        gwg->GetSpecObj<noBuilding>(pos)->OpenDoor(); // Dann die TÃ¼r aufmachen

    // Ist der Platz schon besetzt, wo wir hinlaufen wollen und laufen wir auf StraÃƒÆ’Ã‚Å¸en?
...
void noFigure::WalkToGoal()
{
    ...
    // StraÃƒÆ’Ã‚Å¸e abgelaufen oder noch gar keine StraÃƒÆ’Ã‚Å¸e vorhanden?
    if(((cur_rs) ? (rs_pos == cur_rs->GetLength()) : true))
    {
        ...
        else
        {
            ...
            else if(route == SHIP_DIR)
            {
                ...
                // Uns in den Hafen einquartieren
                cur_rs = NULL; // wir laufen nicht mehr auf einer StraÃƒÆ’Ã‚Å¸e
                gwg->RemoveFigure(this, pos);
            ...
            // NÃ¤chste StraÃƒÆ’Ã‚Å¸e wollen, auf der man geht
...
/*void noFigure::WalkToGoalFailedTrade()
{
    ...
    if(((cur_rs)?(rs_pos == cur_rs->GetLength()):true))
    {
        ...
        else
        {
            ...
            // Oder mÃ¼ssen wir das Schiff nehmen? shouldnt happen for lost wanderers
            else if(route == SHIP_DIR)
            {
                // Uns in den Hafen einquartieren
                noBase * nob;
...
void noFigure::HandleEvent(const unsigned int id)
{
    ...
    else
    {
        ...
        // Alte Richtung und Position fÃ¼r die Berechnung der Sichtbarkeiten merken
...
void noFigure::Wander()
{
    ...
    if(wander_way != 0xFFFF)
    {
        ...
        if(!wander_way)
        {
            ...
            if(best_flag)
            {
                // bestmÃ¶gliche schlieÃƒÆ’Ã‚Å¸lich nehmen
                wander_way = 0xFFFF;
...
//          fs = FS_GOTOGOAL;
//          goal = wh;
//          // Vorgaukeln, dass wir ein StÃ¼ck StraÃƒÆ’Ã‚Å¸e bereits geschafft haben
...
/// Wird aufgerufen, wenn die StraÃƒÅ¸e unter der Figur geteilt wurde (fÃ¼r abgeleitete Klassen)
...
/// Zeichnet standardmÃ¤ÃŸig die Figur, wenn sie lÃ¤uft aus einem bestimmten normalen LST Archiv
