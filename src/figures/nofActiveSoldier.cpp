...
void nofActiveSoldier::GoalReached()
{
    ...
    if(!building)
    {
        ...
        if(building)
            LOG.writeCFormat("nofActiveSoldier::GoalRoached() - no valid 'building' but found one at soldier's position (%i,%i) (gf: %u)\n", pos.x, pos.y, GetEvMgr().GetCurrentGF());
        else
        {
            LOG.writeCFormat("nofActiveSoldier::GoalRoached() - no valid 'building' also didn't find one at soldier's position (%i,%i) (gf: %u)\n", pos.x, pos.y, GetEvMgr().GetCurrentGF());
