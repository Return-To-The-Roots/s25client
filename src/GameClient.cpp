...
GameClient::GameClient()
    : skiptogf(0), playerId_(0), recv_queue(&GameMessage::create_game), send_queue(&GameMessage::create_game), state(CS_STOPPED),
      ci(NULL), replay_mode(false), game_log(NULL)
{
    clientconfig.Clear();
    framesinfo.Clear();
    randcheckinfo.Clear();
...
bool GameClient::Connect(const std::string& server, const std::string& password, ServerType servertyp, unsigned short port, bool host, bool use_ipv6)
{
    ...
    {
        LOG.writeCFormat("GameClient::Connect: ERROR: Connect failed!\n");
        return false;
    }
...
void GameClient::Stop()
{
    ...
    state = CS_STOPPED;
    LOG.writeCFormat("client state changed to stop\n");
}
...
{
    LOG.writeCFormatToFile("<<< NMS_PLAYER_NEW(%d)\n", msg.player);
    RTTR_Assert(state == CS_CONFIG);
...
{
    LOG.writeCFormatToFile("<<< NMS_PLAYER_READY(%d, %s)\n", msg.player, (msg.ready ? "true" : "false"));
...
{
    LOG.writeCFormatToFile("<<< NMS_PLAYER_KICKED(%d, %d, %d)\n", msg.player, msg.cause, msg.param);
...
{
    LOG.writeCFormatToFile("<<< NMS_PLAYER_SWAP(%u, %u)\n", msg.player, msg.player2);
...
void GameClient::OnGameMessage(const GameMessage_Server_Async& msg)
{
    ...
    LOG.writeCFormat(_("The Game is not in sync. Checksums of some players don't match."));
    LOG.writeCFormat(checksum_list.str().c_str());
    LOG.writeCFormat("\n");
    ...
    LOG.writeCFormat("Async log saved at \"%s\", game saved at \"%s\"\n", filePathLog.c_str(), filePathSave.c_str());
}
...
{
    RTTR_Assert(state == CS_CONNECT);
    LOG.writeCFormatToFile("<<< NMS_MAP_DATA(%u)\n", msg.data.size());
    if(msg.isMapData)
    ...
    if(isCompleted)
    {
        ...
        switch(mapinfo.type)
        {
            case MAPTYPE_OLDMAP:
            {
                ...
                {
                    LOG.writeCFormat("GameClient::OnMapData: ERROR: Map \"%s\", couldn't load header!\n", mapinfo.filepath.c_str());
                    if(ci)
                        ci->CI_Error(CE_WRONGMAP);
                    Stop();
                    return;
        ...
        LOG.writeCFormatToFile(">>>NMS_MAP_CHECKSUM(%u)\n", mapinfo.mapChecksum);
    }
}
...
{
    LOG.writeCFormatToFile("<<< NMS_MAP_CHECKSUM(%d)\n", msg.correct ? 1 : 0);
...
void GameClient::OnGameMessage(const GameMessage_GameCommand& msg)
{
    ...
    if(msg.player >= gw->GetPlayerCount())
        return;
    LOG.writeCFormatToFile("CLIENT <<< GC %u\n", msg.player);
...
void GameClient::OnGameMessage(const GameMessage_Server_NWFDone& msg)
{
    ...
    //LOG.writeCFormatToFile("framesinfo.gf_nr(%d) == framesinfo.gfNrServer(%d)\n", framesinfo.gf_nr, framesinfo.gfNrServer);
}
...
void GameClient::OnGameMessage(const GameMessage_Pause& msg)
{
    ...
    LOG.writeCFormatToFile("<<< NMS_NFC_PAUSE(%u)\n", framesinfo.pause_gf);
...
void GameClient::WriteReplayHeader(const unsigned random_init)
{
    ...
    if(!replayinfo.replay.WriteHeader(fileName, mapinfo))
        LOG.writeCFormat("GameClient::WriteReplayHeader: WARNING: File couldn't be opened. Don't use a replayinfo.replay.\n");
}
...
bool GameClient::StartReplay(const std::string& path)
{
    ...
    {
        LOG.writeCFormat(_("Invalid Replay!"));
        if(ci)
            ci->CI_Error(CE_WRONGMAP);
        return false;
    }
    ...
    switch(mapinfo.type)
    {
        ...
        case MAPTYPE_OLDMAP:
        {
            ...
            {
                LOG.writeCFormat(_("Error decompressing map file"));
                if(ci)
                    ci->CI_Error(CE_WRONGMAP);
                Stop();
                return false;
            ...
            if(mapinfo.luaData.length)
            {
                ...
                {
                    LOG.writeCFormat(_("Error decompressing lua file"));
                    if(ci)
                        ci->CI_Error(CE_WRONGMAP);
                    Stop();
                    return false;
    ...
    {
        LOG.writeCFormat(_("Error when loading game from replay: %s\n"), error.what());
        if(ci)
            ci->CI_Error(CE_WRONGMAP);
        Stop();
        return false;
...
void GameClient::ServerLost()
{
    ...
    if(state != CS_STOPPED)
        LOG.writeCFormat("client forced to stop\n");
...
void GameClient::SkipGF(unsigned gf, GameWorldView& gwv)
{
    ...
    if(!replay_mode)
    {
        ...
        skiptogf = gf;
        LOG.writeCFormat("jumping from gf %i to gf %i \n", GetGFNumber(), gf);
        return;
    }
    ...
    for(unsigned i = GetGFNumber(); i < gf;++i)
    {
        ...
        //LOG.writeCFormat(("jumping: now at gf %i\n", framesinfo.nr);        
    }
