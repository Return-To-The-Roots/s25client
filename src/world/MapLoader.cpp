    ...
    RTTR_FOREACH_PT(MapPoint, world.GetWidth(), world.GetHeight())
    {
        ...
        case 0xC4:
        {
            if(lc >= 0x30 && lc <= 0x3D)
                ...
                LOG.writeCFormat("Unbekannter Baum1-4 auf x=%d, y=%d: (0x%0X)\n", pt.x, pt.y, lc);
        } break;
        ...
        case 0xC5:
        {
            if(lc >= 0x30 && lc <= 0x3D)
                ...
                LOG.writeCFormat("Unbekannter Baum5-8 auf x=%d, y=%d: (0x%0X)\n", pt.x, pt.y, lc);
        } break;
        ...
        case 0xC6:
        {
            if(lc >= 0x30 && lc <= 0x3D)
                ...
                LOG.writeCFormat("Unbekannter Baum9 auf x=%d, y=%d: (0x%0X)\n", pt.x, pt.y, lc);
        } break;
        ...
        case 0xC9: // Note: 0xC9 is actually a bug and should be 0xC8. But the random map generator produced that...
        {
            ...
            // Cave
                ...
                LOG.writeCFormat("Unbekanntes Naturzeug auf x=%d, y=%d: (0x%0X)\n", pt.x, pt.y, lc);
        ...
        case 0xCC:
        {
            if(lc >= 0x01 && lc <= 0x06)
                ...
                LOG.writeCFormat("Unbekannter Granit1 auf x=%d, y=%d: (0x%0X)\n", pt.x, pt.y, lc);
        } break;
        ...
        case 0xCD:
        {
            if(lc >= 0x01 && lc <= 0x06)
                ...
                LOG.writeCFormat("Unbekannter Granit2 auf x=%d, y=%d: (0x%0X)\n", pt.x, pt.y, lc);
        } break;
        ...
        default:
            ...
            LOG.writeCFormat("Unbekanntes Objekt (0x%0X) auf x=%d, y=%d: (0x%0X)\n", unknownObj, pt.x, pt.y, lc);
#endif // !NDEBUG
            break;
        }
...
void MapLoader::PlaceAnimals(const glArchivItem_Map& map)
{
    ...
    RTTR_FOREACH_PT(MapPoint, world.GetWidth(), world.GetHeight())
    {
        ...
        default:
            ...
            LOG.writeCFormat("Unknown animal species at x=%d, y=%d: (0x%0X)\n", pt.x, pt.y, unknownAnimal, unknownAnimal);
#endif // !NDEBUG
...
bool MapLoader::PlaceHQs(World& world, std::vector<MapPoint> hqPositions, const std::vector<Nation>& playerNations, bool randomStartPos)
{
    ...
    for(unsigned i = 0; i < playerNations.size(); ++i)
    {
        ...
        {
            LOG.writeCFormat(_("Player %u does not have a valid start position!"), i);
            return false;
        }
...
void MapLoader::InitSeasAndHarbors(World& world, const std::vector<MapPoint>& additionalHarbors)
{
    ...
    for(std::vector<HarborPos>::iterator it = world.harbor_pos.begin() + 1; it != world.harbor_pos.end();)
    {
        ...
        if(!foundCoast)
        {
            LOG.writeCFormat("Map Bug: Found harbor without coast at %u:%u. Removing!\n", it->pos.x, it->pos.y);
