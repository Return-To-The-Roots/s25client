...
void iwHQ::Msg_Group_ButtonClick(const unsigned int group_id, const unsigned int ctrl_id)
{
    if(group_id == 102)
    {
        ...
        if(ctrl_id >= 11 && ctrl_id < 16)
        {
            ...
            GAMECLIENT.AddGC(new gc::ChangeReserve(wh->GetX(), wh->GetY(), ctrl_id - 11, wh->DecreaseReserveVisual(ctrl_id - 11)));
        }
        // Plus-Button
        else if(ctrl_id >= 16 && ctrl_id < 21)
        {
            // Netzwerk-Nachricht generieren
            GAMECLIENT.AddGC(new gc::ChangeReserve(wh->GetX(), wh->GetY(), ctrl_id - 16, wh->IncreaseReserveVisual(ctrl_id - 16)));
        }
    }
