        ...
        node('master') {
            ...
            dir('build') {
                sh """set -x
                      ...
                      docker run -it -u jenkins -v \$(pwd):/workdir -v /srv/apache2/siedler25.org/nightly:/www -v /srv/backup/www/s25client:/archive ubuntu/crossbuild:precise -c "cd build ; ls -la . ; ./cmake.sh --prefix=. \$BARCH -DRTTR_USE_STATIC_BOOST=ON && make create_nightly"
                   """
            }
