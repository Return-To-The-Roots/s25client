...
BOOST_AUTO_TEST_CASE(MakeValid_sets_default_name_and_author)
{
    ...
    settings.MakeValid();
    BOOST_TEST_REQUIRE(settings.name == "Random");
    BOOST_TEST_REQUIRE(settings.author == "AutoGenerated");
}
...
BOOST_AUTO_TEST_CASE(MakeValid_ensures_valid_number_of_players)
{
    ...
    settings.MakeValid();
    BOOST_TEST_REQUIRE(settings.numPlayers == 1u);

    settings.numPlayers = MAX_PLAYERS + 1;
    settings.MakeValid();
    BOOST_TEST_REQUIRE(settings.numPlayers == MAX_PLAYERS);
}
...
BOOST_AUTO_TEST_CASE(MakeValid_ensures_map_size_is_even)
{
    ...
    settings.MakeValid();
    BOOST_TEST_REQUIRE(settings.size.x % 2u == 0u);
    BOOST_TEST_REQUIRE(settings.size.y % 2u == 0u);
}
...
BOOST_AUTO_TEST_CASE(MakeValid_ensures_minimum_size_is_16_x_16)
{
    ...
    settings.MakeValid();
    BOOST_TEST_REQUIRE(settings.size == MapExtent(16, 16));
}
...
BOOST_AUTO_TEST_CASE(MakeValid_ensures_total_ratio_for_mountain_resources_is_at_least_1)
{
    ...
    settings.MakeValid();
    BOOST_TEST_REQUIRE(settings.ratioCoal + settings.ratioGold + settings.ratioIron + settings.ratioGranite > 0);
}
